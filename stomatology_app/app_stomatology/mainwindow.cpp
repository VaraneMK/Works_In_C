...
void MainWindow::searchRecords()
{
    ...
    {
        query.prepare("SELECT * FROM data.timetable WHERE date_timetable = :1 AND id_doctor IN(SELECT id_doctor FROM data.doctor WHERE second_name LIKE :2)");
        query.bindValue(":1",date);
        query.bindValue(":2",doc);
    }
    else // если дата,специальность и фамилия
    {
        query.prepare("SELECT * FROM data.timetable WHERE date_timetable = :1 AND id_doctor IN(SELECT id_doctor FROM data.doctor WHERE id_specialization = :2 AND second_name LIKE :3)");
        query.bindValue(":1",date);
        query.bindValue(":2",id_specialisation);
